    ...
    public actual companion object {
        public actual fun credential(providerId: String, accessToken: String?, idToken: String?, rawNonce: String?): OAuthCredential = rethrow {
            JsOAuthProvider(providerId)
                .credential(
                    oAuthCredentialOptionsToJsAny(accessToken, idToken, rawNonce),
                    accessToken,
                )
...
private fun customParametersToJsAny(map: Map<String, String>): JsAny {
    val obj = emptyJsObject()
    map.forEach { (key, value) -> jsObjectSet(obj, key, value) }
    return obj
}

private fun emptyJsObject(): JsAny = js("({})")
private fun jsObjectSet(obj: JsAny, key: String, value: String): Unit = js("obj[key] = value")

private fun oAuthCredentialOptionsToJsAny(accessToken: String?, idToken: String?, rawNonce: String?): JsAny = js(
