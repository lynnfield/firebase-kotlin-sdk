...
import dev.gitlive.firebase.firestore.internal.toJsAny
import dev.gitlive.firebase.firestore.internal.toKotlinList
...
public actual data class FirebaseFirestoreSettings(
    ...
    internal val js: JsAny get() = mapOf(
        "ssl" to sslEnabled,
        "host" to host,
        "localCache" to when (cacheSettings) {
            is LocalCacheSettings.Persistent -> persistentLocalCache(
                mapOf("cacheSizeBytes" to cacheSettings.sizeBytes).toJsAny()!!.unsafeCast<PersistentCacheSettings>(),
            )
            is LocalCacheSettings.Memory -> {
                val garbageCollectorSettings = when (val garbageCollectorSettings = cacheSettings.garbaseCollectorSettings) {
                    ...
                    is MemoryGarbageCollectorSettings.LRUGC -> memoryLruGarbageCollector(mapOf("cacheSizeBytes" to garbageCollectorSettings.sizeBytes).toJsAny())
                }
                memoryLocalCache(mapOf("garbageCollector" to garbageCollectorSettings).toJsAny()!!.unsafeCast<MemoryCacheSettings>())
            }
        },
    ).toJsAny()!!
}
...
internal inline fun <R> rethrow(function: () -> R): R {
    try {
        ...
        throw errorToException(e.thrownValue)
    }
}

internal external interface FirebaseErrorLike : JsAny {
    val code: String?
    val message: String?
}

internal fun errorToException(e: JsAny?): FirebaseFirestoreException {
    val errorLike = e?.unsafeCast<FirebaseErrorLike>()
    val description = (errorLike?.code ?: errorLike?.message ?: "").lowercase()
    val code = when {
        "cancelled" in description -> FirestoreExceptionCode.CANCELLED
        "invalid-argument" in description -> FirestoreExceptionCode.INVALID_ARGUMENT
        "deadline-exceeded" in description -> FirestoreExceptionCode.DEADLINE_EXCEEDED
        "not-found" in description -> FirestoreExceptionCode.NOT_FOUND
        "already-exists" in description -> FirestoreExceptionCode.ALREADY_EXISTS
        "permission-denied" in description -> FirestoreExceptionCode.PERMISSION_DENIED
        "resource-exhausted" in description -> FirestoreExceptionCode.RESOURCE_EXHAUSTED
        "failed-precondition" in description -> FirestoreExceptionCode.FAILED_PRECONDITION
        "aborted" in description -> FirestoreExceptionCode.ABORTED
        "out-of-range" in description -> FirestoreExceptionCode.OUT_OF_RANGE
        "unimplemented" in description -> FirestoreExceptionCode.UNIMPLEMENTED
        "internal" in description -> FirestoreExceptionCode.INTERNAL
        "unavailable" in description -> FirestoreExceptionCode.UNAVAILABLE
        "data-loss" in description -> FirestoreExceptionCode.DATA_LOSS
        "unauthenticated" in description -> FirestoreExceptionCode.UNAUTHENTICATED
        else -> FirestoreExceptionCode.UNKNOWN
    }
    return FirebaseFirestoreException(Exception(errorLike?.message ?: description), code)
}
