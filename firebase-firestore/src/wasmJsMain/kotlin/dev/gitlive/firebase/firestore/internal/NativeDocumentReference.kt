            ...
            encodedFieldsAndValues.takeUnless { encodedFieldsAndValues.isEmpty() }
                ?.performUpdate(
                    updateAsField = { field, value, moreFieldsAndValues ->
                        updateDoc(js, field, value.toJsAny(), *moreFieldsAndValues.map { it.toJsAny() }.toTypedArray())
                    },
                    updateAsFieldPath = { fieldPath, value, moreFieldsAndValues ->
                        updateDoc(js, fieldPath, value.toJsAny(), *moreFieldsAndValues.map { it.toJsAny() }.toTypedArray())
                    },
                )
                ?.await<JsAny?>()
            Unit
