...
internal actual class NativeFirebaseFirestoreWrapper internal constructor(
    ...
    internal actual constructor(native: NativeFirebaseFirestore, databaseId: String?) : this(
        { settings ->
            settings?.let {
                NativeFirebaseFirestore(initializeFirestore(native.js.app, settings.js, databaseId))
            } ?: native
        },
        databaseId,
    )
    ...
    actual suspend fun <T> runTransaction(func: suspend NativeTransaction.() -> T) = rethrow {
        ...
        dev.gitlive.firebase.firestore.externals.runTransaction(
            ...
            { transaction ->
                GlobalScope.promise {
                    result = NativeTransaction(transaction).func()
                    null
                }
            },
        ).await<JsAny?>()
    ...
    actual suspend fun disableNetwork() {
        rethrow { dev.gitlive.firebase.firestore.externals.disableNetwork(js).await<JsAny?>() }
    }

    actual suspend fun enableNetwork() {
        rethrow { dev.gitlive.firebase.firestore.externals.enableNetwork(js).await<JsAny?>() }
    }

    actual suspend fun terminate() {
        rethrow { dev.gitlive.firebase.firestore.externals.terminate(js).await<JsAny?>() }
    }

    actual suspend fun waitForPendingWrites() {
        rethrow { dev.gitlive.firebase.firestore.externals.waitForPendingWrites(js).await<JsAny?>() }
    }
}
