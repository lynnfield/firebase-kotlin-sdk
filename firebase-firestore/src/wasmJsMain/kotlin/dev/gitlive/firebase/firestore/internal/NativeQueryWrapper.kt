...
import dev.gitlive.firebase.firestore.externals.AggregateQuerySnapshot
...
internal actual open class NativeQueryWrapper internal actual constructor(actual open val native: NativeQuery) {
    ...
    actual suspend fun count(): Long = rethrow {
        getCountFromServer(js).await<AggregateQuerySnapshot>().data().count!!.toLong()
    }
    ...
    private suspend fun aggregateDouble(aggregateField: AggregateField): Double? = rethrow {
        val snapshot = getAggregateFromServer(js, mapOf("result" to aggregateField).toJsAny()!!).await<AggregateQuerySnapshot>()
    ...
    private fun Filter.toQueryConstraint(): QueryConstraint = when (this) {
        ...
        is Filter.Field -> {
            val value = when (constraint) {
                ...
                is WhereConstraint.ForArray -> constraint.values
            }
            dev.gitlive.firebase.firestore.externals.where(field, constraint.filterOp, value.toJsAny())
        }
        is Filter.Path -> {
            val value = when (constraint) {
                is WhereConstraint.ForNullableObject -> constraint.value
            ...
            dev.gitlive.firebase.firestore.externals.where(path.js, constraint.filterOp, value.toJsAny())
        }
    }
    ...
    actual fun startAfter(vararg fieldValues: Any) = rethrow {
        query(
            js,
            dev.gitlive.firebase.firestore.externals.startAfter(*fieldValues.map { it.toJsAny() }.toTypedArray()),
        ).wrapped
    }
    ...
    actual fun startAt(vararg fieldValues: Any) = rethrow {
        query(
            js,
            dev.gitlive.firebase.firestore.externals.startAt(*fieldValues.map { it.toJsAny() }.toTypedArray()),
        ).wrapped
    }
    ...
    actual fun endBefore(vararg fieldValues: Any) = rethrow {
        query(
            js,
            dev.gitlive.firebase.firestore.externals.endBefore(*fieldValues.map { it.toJsAny() }.toTypedArray()),
        ).wrapped
    }
    ...
    actual fun endAt(vararg fieldValues: Any) = rethrow {
        query(
            js,
            dev.gitlive.firebase.firestore.externals.endAt(*fieldValues.map { it.toJsAny() }.toTypedArray()),
        ).wrapped
    }
    ...
    actual fun snapshots(includeMetadataChanges: Boolean) = callbackFlow {
        val unsubscribe = rethrow {
            onSnapshot(
                js,
                mapOf("includeMetadataChanges" to includeMetadataChanges).toJsAny()!!,
                { trySend(QuerySnapshot(it)) },
                { close(errorToException(it)) },
            )
        }
