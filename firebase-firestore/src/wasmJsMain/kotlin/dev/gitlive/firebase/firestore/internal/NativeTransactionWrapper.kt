...
import dev.gitlive.firebase.internal.wasmJs
...
internal actual class NativeTransactionWrapper internal actual constructor(actual val native: NativeTransaction) {
    ...
    actual fun setEncoded(
        ...
        js.set(documentRef.js, encodedData.wasmJs.toJsAny()!!, setOptions.js)
    }
        .let { this }

    actual fun updateEncoded(documentRef: DocumentReference, encodedData: EncodedObject): NativeTransactionWrapper = rethrow { js.update(documentRef.js, encodedData.wasmJs.toJsAny()!!) }
        .let { this }
    ...
    actual fun updateEncoded(
        ...
        encodedFieldsAndValues.performUpdate(
            updateAsField = { field, value, moreFieldsAndValues ->
                js.update(documentRef.js, field, value.toJsAny(), *moreFieldsAndValues.map { it.toJsAny() }.toTypedArray())
            },
            updateAsFieldPath = { fieldPath, value, moreFieldsAndValues ->
                js.update(documentRef.js, fieldPath, value.toJsAny(), *moreFieldsAndValues.map { it.toJsAny() }.toTypedArray())
            },
        )
    }.let { this }
    ...
    actual suspend fun get(documentRef: DocumentReference) = rethrow { NativeDocumentSnapshotWrapper(js.get(documentRef.js).await<dev.gitlive.firebase.firestore.externals.DocumentSnapshot>()) }
}
